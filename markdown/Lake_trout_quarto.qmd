...
Mortality is an important variable in the calculation. If we have any way to ground truth these estimates it would be helpful. If we don't this could be a quantity we try to identify in future studies.
...
-   In instances where we have abundant data it might be appropriate to just estimate our own equation to replace the equation they use. Without knowing much about our data availability it seem likely this approach could work for asymptotic length and (possibly) weight.
...
The other issue I think we need to consider is how to verify the model is making accurate estimates for our lakes. I can see two paths here:

-   You could think of the approach outlined above as using the data we have available to "train" the model to fit our lakes. An approach which is conceptually separate from that outline above would be to hold our data out of the model and use it as a "testing" set to verify the reliability of the model for our situation. James and Mackenzie will likely have to use their discretion to decide which approach is most appropriate given the data available.

-   An important set which I view somewhat separately form most of what was said above would be to use SWHS records for these stocks (or any direct estimate of biomass we might have) to ground truth the estimate of MSY for the lake where we have harvest information..

### How to not abuse the Lester model

I think it's important to note that Lester et al. intended this model be be a regional scale diagnostic because they felt the MSY estimates were to variable to be of much use. The model is mostly heuristic in that not estimates of variability are produced and we really cant say much about the quality of it's estimates. I hear staff talking about wanting to use these estimates of MSY to modify regulations on individual lakes. I'd urge cation here and think hard about how to make decisions for groups of similar lakes. The Lester paper gives some instruction on how they thought this should be done.

I also think we need to have some careful though on the fraction of MSY we want to harvest and if we want that fraction to change depending on our ability to identify problems and manage fisheries for groups of similar lakes.
